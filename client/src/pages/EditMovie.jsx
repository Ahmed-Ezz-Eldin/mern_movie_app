...
import { updateMovieSchema } from '../schema/updateMovieSchema';
...
const EditMovie = () => {
  ...
  const {
    ...
    reset,
    watch,
    formState: { errors, isValid }, // Added isValid and isDirty
  } = useForm({
  ...
  useEffect(() => {
    const getMovieData = async () => {
      try {
        ...
        setPreview(movie.posterImg?.url || null);
        setFetching(false);
      } catch (err) {
        ...
        <div className="flex items-center justify-between p-8 border-b border-slate-800 bg-slate-900/60">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-3xl font-black">
              Edit <span className="text-blue-500">Movie</span>
            </h2>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-400">Video Content</span>
              </div>
              <input
                type="file"
                {...register('videoUrl')}
            ...
            <button
              type="submit"
              // Disable if Redux is loading OR if the Zod validation hasn't passed
            ...
            {!isValid && (
              <p className="text-center text-xs text-slate-500 italic">
                Please ensure all fields are correctly filled to enable update.
              </p>
            )}
          </div>
        </form>
